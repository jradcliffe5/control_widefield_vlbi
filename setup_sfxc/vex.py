# Copyright (C) 2023 Jack Radcliffe
...
def p_scan_block(t):
    'scan_block : SCAN word SEMICOLON scan_block_lines ENDSCAN SEMICOLON'
    t[0] = dict()
    t[0][str(t[2])] = t[4]
    return
...
def p_scan_block_lines(t):
    '''scan_block_lines :
                   | scan_block_lines block_line'''
    if len(t) == 1:
        t[0] = dict()
    else:
        t[0] = t[1]
        if list(t[2].keys())==['source']:
            if 'source' in list(t[0].keys()):
